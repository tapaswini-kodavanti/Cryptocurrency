...
# Creating request and pulling data
request = requests.get(ticker_url)
results = request.json()

# Dictionary of cryptocurrency data
data = results['data']
...
while True:
    ...
    coincap_id = ticker_url_pairs[choice]
    index = searchData(data, coincap_id)
    currency = data[index]

    # json.dumps() turns a Python object into a JSON string that can be used by humans
    # json.dumps(Python object, sorting by keys, indent for pretty printing)
    ...
    try:
        percent_coins_in_circulation = int(circulating_supply/total_supply * 100)
